...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_addban", CommandAddBan, ADMFLAG_BAN, "sm_addban <time> <steamid> [reason]", "sourcebans");
...
public SelectBanIpCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	if (serverID == -1)
	{
		FormatEx(Query, sizeof(Query), "INSERT INTO %s_bans (type, ip, name, created, ends, length, reason, aid, adminIp, sid, country) VALUES \
						(1, '%s', '', UNIX_TIMESTAMP(), UNIX_TIMESTAMP() + %d, %d, '%s', (SELECT aid FROM %s_admins WHERE (authid = '%s' OR authid REGEXP '^STEAM_[0-9]:%s$')) AND aid in (SELECT admin_id FROM %s_admins_servers_groups WHERE server_id = '%d')), '%s', \
						(SELECT sid FROM %s_servers WHERE ip = '%s' AND port = '%s' LIMIT 0,1), ' ')",
			DatabasePrefix, ip, (minutes * 60), (minutes * 60), banReason, DatabasePrefix, adminAuth, adminAuth[8], DatabasePrefix, serverID, adminIp, DatabasePrefix, ServerIp, ServerPort);
	} else {
		FormatEx(Query, sizeof(Query), "INSERT INTO %s_bans (type, ip, name, created, ends, length, reason, aid, adminIp, sid, country) VALUES \
						(1, '%s', '', UNIX_TIMESTAMP(), UNIX_TIMESTAMP() + %d, %d, '%s', (SELECT aid FROM %s_admins WHERE (authid = '%s' OR authid REGEXP '^STEAM_[0-9]:%s$')) AND aid in (SELECT admin_id FROM %s_admins_servers_groups WHERE server_id = '%d'), '%s', \
						%d, ' ')",
			DatabasePrefix, ip, (minutes * 60), (minutes * 60), banReason, DatabasePrefix, adminAuth, adminAuth[8], DatabasePrefix, serverID, adminIp, serverID);
	}
	//PrintToServer(Query);
...
public SelectUnbanCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	if (hndl != INVALID_HANDLE && SQL_FetchRow(hndl))
	{
		...
		Format(query, sizeof(query), "UPDATE %s_bans SET RemovedBy = (SELECT aid FROM %s_admins WHERE (authid = '%s' OR authid REGEXP '^STEAM_[0-9]:%s$') AND aid in (SELECT admin_id FROM %s_admins_servers_groups WHERE server_id = '%d')), RemoveType = 'U', RemovedOn = UNIX_TIMESTAMP(), ureason = '%s' WHERE bid = %d",
			DatabasePrefix, DatabasePrefix, adminAuth, adminAuth[8], DatabasePrefix, serverID, unbanReason, bid);

		//PrintToServer("Linia 1319: %s", query);
...
public SelectAddbanCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	if (serverID == -1)
	{
		FormatEx(Query, sizeof(Query), "INSERT INTO %s_bans (authid, name, created, ends, length, reason, aid, adminIp, sid, country) VALUES \
						('%s', '', UNIX_TIMESTAMP(), UNIX_TIMESTAMP() + %d, %d, '%s', (SELECT aid FROM %s_admins WHERE (authid = '%s' OR authid REGEXP '^STEAM_[0-9]:%s$') AND aid in (SELECT admin_id FROM %s_admins_servers_groups WHERE server_id = '%d')), '%s', \
						(SELECT sid FROM %s_servers WHERE ip = '%s' AND port = '%s' LIMIT 0,1), ' ')",
			DatabasePrefix, authid, (minutes * 60), (minutes * 60), banReason, DatabasePrefix, adminAuth, adminAuth[8], DatabasePrefix, serverID, adminIp, DatabasePrefix, ServerIp, ServerPort);
	} else {
		FormatEx(Query, sizeof(Query), "INSERT INTO %s_bans (authid, name, created, ends, length, reason, aid, adminIp, sid, country) VALUES \
						('%s', '', UNIX_TIMESTAMP(), UNIX_TIMESTAMP() + %d, %d, '%s', (SELECT aid FROM %s_admins WHERE (authid = '%s' OR authid REGEXP '^STEAM_[0-9]:%s$') AND aid in (SELECT admin_id FROM %s_admins_servers_groups WHERE server_id = '%d')), '%s', \
						%d, ' ')",
			DatabasePrefix, authid, (minutes * 60), (minutes * 60), banReason, DatabasePrefix, adminAuth, adminAuth[8], DatabasePrefix, serverID, adminIp, serverID);
	}
			...
			if (serverID == -1)
			{
				FormatEx(query, sizeof(query),
					"INSERT INTO %s_bans (ip, authid, name, created, ends, length, reason, aid, adminIp, sid) VALUES  \
						('%s', '%s', '%s', %d, %d, %d, '%s', (SELECT aid FROM %s_admins WHERE (authid = '%s' OR authid REGEXP '^STEAM_[0-9]:%s$') AND aid in (SELECT admin_id FROM %s_admins_servers_groups WHERE server_id = '%d')), '%s', \
						(SELECT sid FROM %s_servers WHERE ip = '%s' AND port = '%s' LIMIT 0,1))",
					DatabasePrefix, ip, auth, banName, startTime, startTime + time * 60, time * 60, banReason, DatabasePrefix, adminAuth, adminAuth[8], DatabasePrefix, serverID, adminIp, DatabasePrefix, ServerIp, ServerPort);
			}
			else
			{
				FormatEx(query, sizeof(query),
					"INSERT INTO %s_bans (ip, authid, name, created, ends, length, reason, aid, adminIp, sid) VALUES  \
						('%s', '%s', '%s', %d, %d, %d, '%s', (SELECT aid FROM %s_admins WHERE (authid = '%s' OR authid REGEXP '^STEAM_[0-9]:%s$') AND aid in (SELECT admin_id FROM %s_admins_servers_groups WHERE server_id = '%d')), '%s', \
						%d)",
					DatabasePrefix, ip, auth, banName, startTime, startTime + time * 60, time * 60, banReason, DatabasePrefix, adminAuth, adminAuth[8], DatabasePrefix, serverID, adminIp, serverID);
			}
...
stock UTIL_InsertBan(time, const String:Name[], const String:Authid[], const String:Ip[], const String:Reason[], const String:AdminAuthid[], const String:AdminIp[], Handle:Pack)
{
	...
	if (serverID == -1)
	{
		FormatEx(Query, sizeof(Query), "INSERT INTO %s_bans (ip, authid, name, created, ends, length, reason, aid, adminIp, sid, country) VALUES \
						('%s', '%s', '%s', UNIX_TIMESTAMP(), UNIX_TIMESTAMP() + %d, %d, '%s', IFNULL((SELECT aid FROM %s_admins WHERE (authid = '%s' OR authid REGEXP '^STEAM_[0-9]:%s$') AND aid in (SELECT admin_id FROM %s_admins_servers_groups WHERE server_id = '%d')),'0'), '%s', \
						(SELECT sid FROM %s_servers WHERE ip = '%s' AND port = '%s' LIMIT 0,1), ' ')",
			DatabasePrefix, Ip, Authid, banName, (time * 60), (time * 60), banReason, DatabasePrefix, AdminAuthid, AdminAuthid[8], DatabasePrefix, serverID, AdminIp, DatabasePrefix, ServerIp, ServerPort);
	} else {
		FormatEx(Query, sizeof(Query), "INSERT INTO %s_bans (ip, authid, name, created, ends, length, reason, aid, adminIp, sid, country) VALUES \
						('%s', '%s', '%s', UNIX_TIMESTAMP(), UNIX_TIMESTAMP() + %d, %d, '%s', IFNULL((SELECT aid FROM %s_admins WHERE (authid = '%s' OR authid REGEXP '^STEAM_[0-9]:%s$') AND aid in (SELECT admin_id FROM %s_admins_servers_groups WHERE server_id = '%d')),'0'), '%s', \
						%d, ' ')",
			DatabasePrefix, Ip, Authid, banName, (time * 60), (time * 60), banReason, DatabasePrefix, AdminAuthid, AdminAuthid[8], DatabasePrefix, serverID, AdminIp, serverID);
	}
