...
public OnConnectBanCheck(Handle:owner, Handle:hndl, const String:error[], any:userid)
{
    ...
    {
        if(bancount == 0){
            PrintToBanAdmins(" \x04[SBPP]\x07 Warning: Player \x01\"%N\"\x07 has \x01%d\x07 previous \x01commban%s.",
                client,commcount,((commcount > 1 || commcount == 0) ? "s":""));
        }
        else if(commcount == 0){
            PrintToBanAdmins(" \x04[SBPP]\x07 Warning: Player \x01\"%N\"\x07 has \x01%d\x07 previous \x01ban%s.",
                client, bancount, ((bancount > 1 || bancount == 0) ? "s":""));
        }
        else{
            PrintToBanAdmins(" \x04[SBPP]\x07 Warning: Player \x01\"%N\"\x07 has \x01%d\x07 previous \x01ban%s\x07 and \x01%d\x07 previous \x01commban%s.",
...
PrintToBanAdmins(const String:format[], any:...)
{
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		if (IsClientInGame(i) && !IsFakeClient(i)
			 && CheckCommandAccess(i, "sm_listsourcebans", ADMFLAG_KICK)
			)
		{
			PrintToChat(i, "%s", msg);
		}
