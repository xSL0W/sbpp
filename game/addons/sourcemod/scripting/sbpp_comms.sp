	...
	while (SQL_MoreRows(hndl))
	{
		...
		FormatEx(query, sizeof(query),
			"INSERT INTO	%s_comms (authid, name, created, ends, length, reason, aid, adminIp, sid, type) \
				VALUES		('%s', '%s', %d, %d, %d, '%s', \
							(SELECT IFNULL((SELECT aid FROM %s_admins WHERE (authid = '%s' OR authid REGEXP '^STEAM_[0-9]:%s$') AND aid in (SELECT admin_id FROM %s_admins_servers_groups WHERE server_id = '%d')), '0')), \
							'%s', %d, %d)",
			DatabasePrefix, sAuthEscaped, banName, startTime, (startTime + (time * 60)), (time * 60), banReason, DatabasePrefix, sAdmAuthEscaped, sAdmAuthYZEscaped, DatabasePrefix, serverID, adminIp, serverID, type);
		#if defined LOG_QUERIES
...
stock ProcessUnBlock(client, targetId = 0, type, String:sReason[] = "", const String:sArgs[] = "")
{
	...
	else
	{
		...
		if (DB_Connect())
		{
			...
			Format(query, sizeof(query),
				"SELECT		c.bid, \
							(SELECT IFNULL((SELECT aid FROM %s_admins WHERE (authid = '%s' OR authid REGEXP '^STEAM_[0-9]:%s$') AND aid in (SELECT admin_id FROM %s_admins_servers_groups WHERE server_id = '%d')), '0')) as iaid, \
							c.aid, \
							IF (a.immunity>=g.immunity, a.immunity, IFNULL(g.immunity,0)) as immunity, \
							c.type \
				FROM		%s_comms	 AS c \
				...
				WHERE		RemoveType IS NULL \
								...
								AND (length = '0' OR ends > UNIX_TIMESTAMP()) \
								AND %s",
				DatabasePrefix, sAdminAuthEscaped, sAdminAuthYZEscaped, DatabasePrefix, serverID, DatabasePrefix, DatabasePrefix, DatabasePrefix, sTargetAuthEscaped, sTargetAuthYZEscaped, typeWHERE);
...
stock SavePunishment(admin = 0, target, type, length = -1, const String:reason[] = "")
{
	...
	if (DB_Connect())
	{
		...
		FormatEx(sQueryAdm, sizeof(sQueryAdm),
			"IFNULL((SELECT aid FROM %s_admins WHERE (authid = '%s' OR authid REGEXP '^STEAM_[0-9]:%s$') AND aid in (SELECT admin_id FROM %s_admins_servers_groups WHERE server_id = '%d')), 0)",
			DatabasePrefix, sAdminAuthIdEscaped, sAdminAuthIdYZEscaped, DatabasePrefix, serverID);
